...
import Process from '@/components/Process'; // <--- Новый блок
import FAQ from '@/components/FAQ';         // <--- Новый блок
import Footer from '@/components/Footer';
import ContactForm from '@/components/ContactForm';
import { MessageCircle, Send } from 'lucide-react';
import * as motion from "framer-motion/client";

// 1. Данные города (Сочи для главной)
async function getSochiData() {
  return await prisma.cityPage.findUnique({ where: { slug: 'sochi' } });
}

// 2. Список всех городов (для меню)
...
export default async function Home() {
  ...
  if (!data) return <div className="flex h-screen items-center justify-center bg-slate-900 text-white">Загрузка...</div>;
